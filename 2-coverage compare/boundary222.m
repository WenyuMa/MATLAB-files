...
%clc;
%clear;
global node2 node_coor node_x node_y k
...
for i=1: length(node_coor) - 24*k
    ...
    for j=1: length(node_coor)
        if (j==i) continue;
            ...
            if distance <= 2 
                node2(i).simp1 = [node2(i).simp1, j];
                node2(i).dist = [node2(i).dist, distance];
                node2(i).angle = [node2(i).angle, 2*acos(distance/2)];
...
for i = 1: length(node_coor) - 24
    ...
    node2(i).simp1 = node2(i).simp1(index);
    node2(i).dist = node2(i).dist(index);
    node2(i).angle = node2(i).angle(index);
end

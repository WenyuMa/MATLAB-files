...
if node(w).status==1
   no_neighbor = length(node(w).neighbour);
   if no_neighbor
       for i=1:no_neighbor-1    %choose u
         if flag_stop==0  %
           u=node(w).neighbour(i);
           if node(u).status==1 
               for j=i+1:no_neighbor    %choose v
                 if flag_stop==0 %
                   v=node(w).neighbour(j);
                   if node(v).status==1
                       index1=find(node(u).neighbour==v);  %u,v are two neighbors
                       
                       if index1              %u and v are neighbors
                           S2=[]; 
                           cover_type=0; 
                            uv=node(u).dist(index1); %uv 
                            uw=node(w).dist(i); %uw
                            wv=node(w).dist(j); %wv
                            wuv=acos((uv^2+uw^2-wv^2)/(2*uv*uw));
            
                            angle1=node(u).angle(index1);%uv
                            angle2=node(w).angle(i);%uw  
                            ...
                            if cover_type==0   
                                ...
                                for m=1:length(node(u).neighbour)   
                                    index2=find(node(v).neighbour==node(u).neighbour(m)); % find all common neighbors of u and v
                                    
                                    if index2
                                        x=node(v).neighbour(index2);  
                                        if x==w
                                            continue;
                                        elseif node(x).status==1
                                            ux=node(u).dist(m); %ux
                                            vx=node(v).dist(index2); %vx
                                            xuv=acos((uv^2+ux^2-vx^2)/(2*uv*ux)); %xuv                                            
                                            angle3=node(u).angle(m); %ux
                                            ...
                                            elseif cover_type==1
                                                ...
                                                elseif xuv > abs(angle3 - angle1)/2 && xuv <= (angle3 + angle1)/2  %x covers a junction
                                                    index4=find(node(w).neighbour==x);
                                                    
                                                    if index4        %x and w must be neighbors
                                                        wx=node(w).dist(index4);
                                                  ...
                                                  for g=f+1:l
                                                      if flag_pair==1
                                                         break;
                                                          ...
                                                          neibr=find(node(S2(f)).neighbour==S2(g));
                                                          if neibr
                                                              index5=find(node(u).neighbour==S2(f));%uf
                                                              index6=find(node(u).neighbour==S2(g));%ug
                                                              index7=find(node(S2(f)).neighbour==v);%vf
                                                              index8=find(node(S2(g)).neighbour==v);%vg
                                                              index9=neibr;  %fg
                                 
                                                              uf=node(u).dist(index5); %uf 
                                                              ug=node(u).dist(index6); %ug
                                                              vf=node(S2(f)).dist(index7); %vf 
                                                              vg=node(S2(g)).dist(index8); %vg
                                                              fg=node(S2(f)).dist(index9);%wx

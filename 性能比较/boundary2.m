...
% %try to use JPlex to compute betti numbers
% startJPlex;
% s1 = ExplicitStream;
% 
...
% for vertice = 1: length(node_coor)
%     if (isempty(node(vertice).simp1)) continue;
%     end
%     for edge = 1: length(node(vertice).simp1) 
%         if node(vertice).simp1(edge) < vertice continue;
%         else
%             s1.add([vertice,node(vertice).simp1(edge)],[0]);
%         end
%     end
% end
% 
...
% for vertice = 1: length(node_coor)
%     if (isempty(node(vertice).simp2)) continue;
%     end
%     for triangular = 1: size(node(vertice).simp2,1)
%         if vertice < min(node(vertice).simp2(triangular, :))
%             s1.add([vertice, node(vertice).simp2(triangular, :)],[0]);
%         end
%     end
% end
%     

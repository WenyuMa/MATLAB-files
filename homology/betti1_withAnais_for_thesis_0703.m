...
fence_x = [0,0,0,0,0,0,2,2,4,4,6,6,8,8,10,10,10,10,10,10];
fence_y = [0,2,4,6,8,10,0,10,0,10,0,10,0,10,0,2,4,6,8,10];
...
for i=1: length(node_coor)
    neigh_set = node(i).neighbors;
    for j = 1 : length(node(i).neighbors)
        neigh_ID = node(i).neighbors(j);
        if neigh_ID > i
...
%                         delete the node_del_ID from the simplices of node_cur_ID
    ...
    for i = 1 : length(node_coor)
        if node(i).deleted == 1
            ...
            dim_max = size(node(i).simp_temp, 2);
            if dim_max == 2
                no_tri = size(node(i).simp_temp{2}, 2);
                for j = 1 : no_tri
                    vert_set = node(i).simp_temp{2}(j).vert;
                    if i == min(vert_set)
                        node_ID1 = vert_set(1);
                        node_ID2 = vert_set(2);
                        node_ID3 = vert_set(3);
    ...
    for i=1: length(node_coor)
        if node(i).deleted == 1
            ...
            neigh_set = node(i).neighbors_temp;
            for j = 1 : length(node(i).neighbors_temp)
                neigh_ID = node(i).neighbors_temp(j);
                if neigh_ID > i
                    line([node_x(i),node_x(neigh_ID)],[node_y(i),node_y(neigh_ID)], 'Color', 'k'); hold on;
                end
            end
        end
    end
    ...
    for i = 1 : length(node_special_set)
        ...
        if size_matrix == 1
            ...
            plot(node_coor(1,node_ID), node_coor(2,node_ID), 'sr'); hold on;
            line([node_x(node_ID),node_x(neighbor_edge_ID)],[node_y(node_ID),node_y(neighbor_edge_ID)], 'Color','b','LineWidth',2);
...
while(1)
    ...
    for i = 1 : no_cycle_temp-1
        for j = i+1 : no_cycle_temp
            ...
            elseif length(cycle_inter)/length(cycle1) >= 1/2 && length(cycle_inter)/length(cycle2) >= 1/2
                no_common_edge1 = 0;
                no_common_edge2 = 0;
                for k = 1 : length(cycle1)
                    ...
                    if node(node_ID).boundary_degree(post_after) == 2
                        no_common_edge1 = no_common_edge1 + 1;
                    end
                end
                ...
                for k = 1 : length(cycle2)
                    ...
                    post_after = find(node(node_ID).boundary_edge == node_after_ID);
                    if node(node_ID).boundary_degree(post_after) == 2
                        no_common_edge2 = no_common_edge2 + 1;
                    end
                end
                
                if no_common_edge1 > no_common_edge2
                    if ~isAinB(j, delete_cycle)
                        delete_cycle = [delete_cycle, j];
                    end
                elseif no_common_edge1 < no_common_edge2
...
no_cycle_min = size(cycle_min, 2);
for i = 1 : no_cycle_min
    cycle_temp = cycle_min{i};
    for j = 1: length(cycle_temp)
        node_ID1 = cycle_temp(j);
        node_ID2 = cycle_temp(mod(j, length(cycle_temp))+1);
        line([node_x(node_ID1),node_x(node_ID2)],[node_y(node_ID1),node_y(node_ID2)], 'Color', 'r', 'LineWidth',2);
    end
end 

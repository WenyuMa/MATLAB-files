                            ...
                            for n=m+1:length(co_neighb1)
                                ...
                                if index2   %edge wx locates in edge.u(index1(index2))
                                    if node(x).fence_flag==0
                                        ...
                                        if isempty(diff)
                                            % ADD SOME JUDGE HERE
%                                             if length(co_neighb1)<3
%                                                 % EdgeCheck(i,index1(index2));
%                                             end
                                            
                                            
                                            if (length(co_neighb2)>edge_weight)
                                                sleep_temp=index1(index2); % wx 在edge中的位置
                                                edge_weight=length(co_neighb2);
                                            end 
                                        end                                      
                                    end%
                                end
...
%                     sleep_edge=[sleep_edge,sleep_temp];
                    sleep_edge=[sleep_edge,i];
                    edge.c_flag(index1(index2))=0;
                    w=edge.u(index1(index2));
                    x=edge.v(index1(index2));

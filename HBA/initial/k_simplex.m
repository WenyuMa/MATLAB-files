...
for i = 1 : length(node_coor)
    while(1)
        index_max = size(node(i).simp, 2);%最大的单形
        if index_max<4
        no_max_simp = size(node(i).simp{index_max}, 2);%最大单形的最大数目
            ...
            if ~isempty(neighb_set)
                ...
                for k = 1 : no_neighb     %考察第j个最大维单形 邻居集合的每个顶点
                    ...
                    if new_node < max(setdiff(vert_set, i))  %？？？？
                        ...
                        neighb_set_temp = intersect(neighb_set, node(new_node).neighbors);%当前i顶点的邻居集合  和  当前i顶点 第k个邻居顶点的邻居集合  的交集   
